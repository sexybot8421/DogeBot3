...
module.exports = DogeXeonOP = async (DogeXeonOP, mek, _welkom) => {
	try {
		...
		const ownerNumber = [`${owner}@s.whatsapp.net`, `94775978468@s.whatsapp.net`]
...
╰───「 *🥳ʟᴇᴠᴇʟ ᴜᴘ🥳* 」`)
          ...
          sendButton(from, lvlup, '*SILEN BOT*', but)
...
${gaya1} *Bot Name* : ${❤️‍🩹❤️‍😾SILEN BOT😾❤️‍🩹}
${gaya1} *Owner Name* : ${MR,SILEN}
...
{
"title": "OWNER MENU",
"description": `ඒවා මට විතරයි හුත්තෝ😾`,
...
sendButVideo(from, pll, `*SILEN BOT*`, get_result, [      
{
buttonId: `${prefix+command}`,
buttonText: {
displayText: `NEXT ➡️`,
...
//----> 1 TEXT <----//
                ...
                case 'freefire':          
                    if (args.length == 0) return reply(`Example: ${prefix + command} SILEN`)
                    ini_txt = args.join(" ")
...
scsell = `*MY YOUTUBE CHANEL AND GROUP LINK*\n\n_• 𝐘𝐎𝐔𝐓𝐔𝐁𝐄: https://youtube.com/channel/UC162sDfYgqRNrwGSeFCTx3w_\n_• 𝐆𝐑𝐎𝐔𝐏 𝐋𝐈𝐍𝐊: https://chat.whatsapp.com/FYHZCyNuFLoFFy9MYkHApe_\n\n_𝐋𝐈𝐊𝐄 𝐀𝐍𝐃 𝐒𝐔𝐁𝐂𝐑𝐈𝐁𝐄 𝐌𝐘 𝐘𝐎𝐔𝐓𝐔𝐁𝐄 𝐂𝐇𝐀𝐍𝐄𝐋._`
but = [
...
case 'bc':
             ...
             for (let _ of anu100) {
             DogeXeonOP.sendMessage(_.jid, bc100, image, {quoted: fgi, caption: `*「 SILEN Bot Broadcast 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu100) {
             DogeXeonOP.sendMessage(_.jid, 
			{"contentText": `*「 SILEN BOT BROADCASTING MESSAGE 」*\n\n${body.slice(4)}`,
			"footerText": `${tanggal}`,
			"buttons": [
...
reply(`${err}`)
	...
	DogeXeonOP.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *හුත්තෝ🥺 ERROR එකක් ඇවිත්😾 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Dream Guy Xeon",body:"Dont forget to subscribe Xeon",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/dogepic1.jpg'),sourceUrl:"https://wa.me/94775978468"}}})
        }
	// console.log(e)
	}
}
